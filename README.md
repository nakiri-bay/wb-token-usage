...
> 正式名称 **WorkBuddy 积分消耗统计**，下文简称「统计助手」。

一个 Windows 桌面悬浮挂件：人物头顶的气泡实时显示 **今日用量**、**累计用量** 与 **剩余积分**。
...
| 聊天气泡 | 显示 `今日用量` / `累计用量` / `剩余积分` |
| ↻ 一键刷新 | 后台线程跑同步，不弹黑窗、不卡界面 |
| 打开即同步 | 启动统计助手时立刻同步一次，无需再单独启动守护进程 |
| 定时同步 | 之后每 **15 分钟 ± 2 分钟** 随机静默同步一次（避免访问过于规律） |
| 退出即停止 | 退出统计助手时同步线程与浏览器进程一并结束，不留后台残留 |
| 双重数据源 | 官方同步值优先，本地估算值（`points_log.jsonl`）兜底 |
| 单实例保护 | 重复双击只会有一个统计助手，第二次会提示“已在运行” |
...
├── scripts/                      # 启动器（双击即用）
│   ├── 静默启动统计.vbs           #   ★ 启动统计助手（无窗口），同步随之开始
│   ├── 启动统计.bat               #   同上，但保留控制台，便于调试
│   ├── 同步用量.vbs / .bat        #   手动立刻同步一次
│   ├── auto_sync.vbs             #   启动“独立同步守护”（不常开统计助手时用）
│   └── 停止自动同步.bat           #   结束独立同步守护
├── assets/
...
| 文件 | 说明 |
|------|------|
| `official_daily.json` | 官方同步到的数据：`{日期: 积分}` + `"_balance": 剩余积分`（下划线开头为非日期字段） |
| `points_log.jsonl` | 本地记账流水（每行一条 JSON） |
| `sync_log.txt` | 同步日志（统计助手内嵌同步也写这里） |
...
- **剩余积分**取自页面「套餐用量」区的 `可用N个·剩余X积分`（即平台奖励积分池的浮动余额）。
  由于每页快照都含该区块，同一数值会重复出现，解析时**先去重再取最大值**，避免成倍虚高。

## ⚙️ 两种运行模式（二选一）

| 模式 | 入口 | 适合 |
|------|------|------|
| **随统计助手（默认，推荐）** | `scripts/静默启动统计.vbs` | 想要桌面挂件，且希望同步随挂件开关 |
| **独立守护** | `scripts/auto_sync.vbs` | 不想常驻窗口，只要后台默默记录 |

> 两种模式**不要同时跑**：它们会争抢同一个浏览器 profile，导致快照残缺、同步失败。
> 统计助手自带单实例保护，独立守护用 `auto_sync.pid` 做单实例保护。
...
1. **启动**：双击 `scripts/静默启动统计.vbs` —— 统计助手出现，同时自动同步一次。
...
**剩余积分显示 `—` 或数字没更新？**
`—` 表示尚未同步到余额（旧版数据文件里没有该字段，等下一次同步即可）。
若数字明显偏大（比如约 10 倍），说明快照解析未去重——现代码已按“去重后取最大值”处理。
